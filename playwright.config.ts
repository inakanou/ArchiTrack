...
export default defineConfig({
  ...
  // 並列実行設定（環境別最適化）
  // ============================================================================
  // ベストプラクティス: CI環境では並列実行を有効化してテスト時間を短縮
  // - CI環境: シャーディングと組み合わせて並列実行（各シャード内で2ワーカー）
  // - ローカル: データベース競合を防ぐためシリアル実行を維持
  // 注意: CI環境ではDBをテストごとにクリーンアップするため並列実行が可能
  // ============================================================================
  fullyParallel: isCI,
  ...
  // ワーカー数設定
  // ============================================================================
  // - CI環境: シャーディングと組み合わせて各シャード内で2ワーカー
  //   (4シャード × 2ワーカー = 最大8並列)
  // - ローカル: シリアル実行（DB競合防止）
  // ============================================================================
  workers: isCI ? 2 : 1,
