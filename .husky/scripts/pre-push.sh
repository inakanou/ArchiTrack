...
if [ -d "frontend" ]; then
  ...
  echo "   テストを15個のプロジェクトに分割して順次実行します。"
  echo "   各プロジェクト実行後にメモリが解放されるため、OOMクラッシュを防止できます。"
  echo ""

  # プロジェクトリスト（軽量→重い順）
  # Vitest Projects機能により、各プロジェクトは独立したプロセスで実行される
  projects=("lightweight" "api" "contexts" "hooks-auth" "hooks-ui" "hooks-site-survey" "hooks-canvas" "hooks-form" "components-core" "projects" "quantity-table" "site-surveys" "trading-partners" "pages" "integration")
