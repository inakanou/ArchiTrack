...
ArchiTrackは、建設プロジェクトの管理・積算業務を効率化するためのWebアプリケーションです。プロジェクト管理、現場調査、数量拾い出し、内訳書作成、見積依頼・見積書作成から、契約書・工程表・実行予算管理・工事写真台帳までの一連の業務フローをサポートします。Claude Codeを活用したKiro-style Spec Driven Developmentで開発されています。

### 主な特徴

- 🤖 **AI支援開発**: Claude Codeによる体系的な開発ワークフロー
- 📝 **スペック駆動**: 要件定義 → 設計 → タスク分解 → 実装の明確なフェーズ管理
- 🏗️ **建設プロジェクト管理**: 現場調査・数量拾い出し・内訳書・見積依頼・見積書・契約書・工程表・実行予算・工事写真台帳の一気通貫ワークフロー
- 📄 **実務準拠の帳票**: A4横の表紙・内訳書・明細書をクライアントサイド生成（PDF / Excel）
...
| **Reporting** | jsPDF 4 + SheetJS（PDF / Excel帳票生成） |
...
- **見積書作成**: 内訳書からの見積書生成、3行1セット構造（見積金額・実行金額・業者金額）、階層構造管理、クライアントサイド編集・一括保存モデル（挿入・削除・複写・並び替え・ドラッグ&ドロップ・階層の上げ下げをローカル完結し保存1回で確定、楽観ロックによる競合検出、未保存インジケーター・離脱ガード、保存の制限時間超過時の原因表示）、複数行の範囲選択と範囲操作、階層表示モード切替（ツリー表示／ドリルダウン表示・端末単位で引き継ぎ）、階層構造の俯瞰パネル、キーボード操作（行操作・階層移動・セル移動、キー割当一覧の画面参照）、取り消し／やり直し（直近10回）、受領見積書転記（改善ダイアログUI）、NET金額案分計算、利益率適用、諸経費（共通費）自動計算、値引き行（見積金額行のみ・マイナス単価許容・案分/利益率適用対象外）、注記行（名称のみ・集計対象外）、改修工事フラグ（建築新営/改修パターン）、サマリー表示・コンパクトUI、帳票出力（フロントエンド生成のA4横PDF＝表紙・内訳書・明細書、Excel、未保存状態のままプレビュー可能、別途工事・有効期限・提出日の帳票用入力項目）
...
- **工事写真台帳**: プロジェクトに紐付く工事写真アルバムのCRUD・レスポンシブ一覧（Table/Card切替・検索・ページング）、写真項目追加の3系統（ローカルアップロード・カメラ撮影・同一プロジェクト現場調査写真のコピー参照＝独立複製）、写真項目管理（コメント編集・並び替え・印刷対象チェック）、工事看板マスタ（プロジェクト単位・構造化テキスト項目のCRUD、電子黒板SVG描画）、写真項目への工事看板配置（選択・大きさ・表示位置指定＝任意）とオンデマンド合成（サーバsharp）、工事写真台帳PDF出力（クライアントjspdf、印刷対象のみ出力・看板の指定位置重畳）、フルスクリーン画像ビューア（ズーム・90度回転・パン）、画像一括エクスポート（全件/選択画像をZIPダウンロード、設定・進捗表示・キャンセル・部分失敗対応）、権限ベースUI表示制御（RBAC）、未保存変更の離脱ガード、詳細画面のモバイルレスポンシブ対応
