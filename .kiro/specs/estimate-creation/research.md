...
## 設計フェーズ（Light Discovery）の合成結果: REQ-42〜56

ディスカバリ種別: **Light（Extension）**。新規ライブラリの導入なし、外部サービス連携なし。既存インフラへの増分と既存資産の再利用で構成する。

### Research Needed の解消

| # | 項目 | 解消内容 |
|---|---|---|
| 1 | Excelの罫線 | **要件を修正して解消**。REQ-10.5 に「罫線の描画はPDF出力に限る」を追記し、REQ-52 に AC14（Excelは罫線を出力せず列構成・行構成・表題・ページ番号・合計行・階層記号・インデントのみに従う）を新設。ライブラリ追加は行わない |
| 2 | 第2階層以降の階層記号 | **要件の精読で解消**。REQ-53.7 が階層記号を第1階層に限定しているため、第2階層以降は記号を持たない。REQ-52.11 の明細書先頭行は親項目名のみを出力する。`levelSymbol(index, depth)` が `depth === 0` 以外で空文字を返す設計とした |
| 3 | 18行グリッドの行数カウント | **設計で確定**。注記行（`NOTE`）と値引き行（`DISCOUNT`）を通常の明細行1行として数える。要件に別扱いの記載がないため素直な解釈を採用 |
| 4 | 単位の `〃` とページ跨ぎ | **設計で確定**。REQ-53.6 の文言どおり直前の行に対して適用し、継続ページの先頭行でも前ページ最終行と同一単位なら `〃` とする |
| 5 | キー割当の確定 | **設計で方針確定**。ブラウザ標準と衝突するファンクションキーは採用せず、階層移動は `Alt+↑`/`Alt+↓`、行挿入は `Alt+Insert`、範囲選択は `Shift+↑↓` を基準とする。取り消し系は既存の Ctrl/Cmd+Z 系を維持。確定値は `estimateKeymap.entries` に単一定義し `EstimateKeymapHelp` が同じ定義を表示する |
| 6 | 表示モードの引き継ぎ範囲 | **設計で確定**。端末単位（ブラウザ保存）とする。サーバー側の変更を伴わず REQ-45.11 を満たせる |
| 7 | 深さ無制限時の再計算コスト | **設計で方針確定＋計測を残す**。`level`/`path` 列は追加せず隣接リストを維持し、`estimateTree` の導出結果を `useMemo` でキャッシュする。500項目規模の実測を段階1のリリース前に取ることを Performance Tests に明記 |
| 8 | 計算の単一化の方向 | **設計で確定**。案分・利益率をクライアント側の単一実装（`estimateCalculations`）に寄せ、サーバーの `calculate-net` / `apply-profit-rate` を撤去する。理由は REQ-56（未保存プレビュー）と REQ-5.8/6.8（編集中の値での計算）がクライアント計算を必然とするため。サーバー実装を残すと二重実装が残存する。諸経費計算（`calculate-overhead`）は二重実装が存在せず書き込みも行わないため現行のまま維持 |
| 9 | フォントのサブセット化 | **本改訂では行わない**。既存3機能と共有する資産であり、変更は他機能へ波及する。帳票サービスを動的 import で初期ロードから切り離すことでバンドル影響を緩和する |
| 10 | リリース単位 | **設計で確定**。段階1と段階3を同一リリースで揃える。分けて出す場合は段階1に暫定ガード（未保存時の転記ダイアログ起動抑止）を入れる |

### 1. Generalization（一般化）

- **「編集中ツリーを唯一の入力とする計算」への一般化**: REQ-5（案分）・REQ-6（利益率）・REQ-39（サマリー）・REQ-56（未保存プレビュー）は、いずれも「保存済みの値ではなく編集中の値を計算対象とする」という同一の問題の変種である。`estimateCalculations` と `estimateTree` を「編集中ツリーを受け取る純粋関数群」として設計し、案分・利益率・集計・帳票の4用途が同じ入力形を共有する形にした。実装範囲は現要件が求めるものに限り、インターフェースのみ汎用化している。
- **「行操作」の一般化**: REQ-12（単一行操作）と REQ-44（範囲操作）は同じ操作の対象数の違いにすぎない。reducer のアクションを `keys: readonly NodeKey[]` で受ける形に統一し、単一行は要素1件の範囲として扱う。
- **`itemType` を唯一の種別判別軸に維持**: REQ-41（値引き行）で導入した `itemType` に REQ-55（注記行）の `NOTE` を追加する形とし、判別軸を増やさない。第1回分析の決定（種別フィールド方式）を踏襲。

### 2. Build vs. Adopt

| 対象 | 判断 | 根拠 |
|---|---|---|
| 取り消し・やり直し（REQ-48） | **Adopt** — 既存 `frontend/src/services/UndoManager.ts` ＋ `useUndoState` | コマンドパターン実装済み。履歴上限がコンストラクタ引数のため `new UndoManager(10)` で REQ-48.3 を満たす。自作は重複 |
| 文字入力中のキー抑止（REQ-47.5） | **Adopt** — `useUndoKeyboardShortcuts.ts:45` の `isTextInputElement` | 要件の判定条件と一致。共通ユーティリティへ切り出して共用 |
| 帳票の日本語描画（REQ-10.8） | **Adopt** — `PdfFontService` ＋ `fonts/noto-sans-jp-base64.ts` | Noto Sans JP（2,255,812 bytes の TrueType）が既存。バックエンドへ複製する必要をなくす |
| 帳票のダウンロードと進捗（REQ-10.7） | **Adopt** — `PdfExportService.downloadPdf` / `PdfExportProgress` | 現場調査報告書・数量表と同じ導線 |
| 罫線描画（REQ-52） | **方式のみ踏襲** — `QuantityTablePdfExportService` の `doc.rect` / `doc.line` | 列構成が異なるためコードは共有せず方式を踏襲。共有ファイルの変更を避ける |
| フル状態同期保存（REQ-42） | **方式のみ踏襲** — `quantity-table.service.ts:945` `saveDraft` | コード共有は数量表への波及リスクがあるため、見積書用に独立実装する |
| Excelの罫線 | **Build も Adopt もしない** | SheetJS では実現できず、`exceljs` 追加は既存Excel出力（内訳書・工程表・実行予算）との併存を招く。REQ-52.14 で対象外と明確化した |
| 編集状態管理 | **Build** — `estimateEditReducer` | React `useReducer` の範囲で足りる。状態管理ライブラリの導入は不要 |

### 3. Simplification（簡素化）

- **取り消し単位をスナップショットに単純化**: reducer が純粋関数であるため、アクションごとの逆操作を実装せず直前 state を保持するだけで復元できる。`UndoCommand` の `undo`/`redo` に state 復元関数を渡す形とし、操作種別ごとの分岐を作らない。
- **表示状態を保存対象から完全に切り離す**: 表示モード・現在階層・選択範囲・展開状態・カーソル位置をサーバーへ送らないことで、保存ペイロードのスキーマが編集内容だけに閉じる。REQ-45.11 の永続化も端末単位とし、サーバー側の変更をゼロにした。
- **循環参照検証をクライアントに寄せる**: サーバー側 `getDescendantIds` のBFS（深さ分のクエリ、`estimate-item.service.ts:541-560`）を廃止し、クライアントのツリーで判定する。サーバーは受領ツリーの構造検証のみを行う。
- **旧エンドポイントを残さず撤去**: 12本を撤去して書き込み経路を `PUT /:id/save` の1本に集約する。互換維持のための二重メンテを作らない。
- **`level` / `path` 列を追加しない**: 隣接リストのまま導出計算をクライアントに寄せ、スキーマの複雑化と整合性維持のコストを避ける。代償として再計算コストが生じるため、メモ化と実測を設計に明記した。
- **Excel をシート分割しない**: ページ相当の区切りを行方向の連続とし、シート管理の複雑さを持ち込まない。

### 主要リスクと緩和

| リスク | 度合い | 緩和 |
|---|---|---|
| 案分・利益率の計算結果が現行と食い違う | **High** | 移行前に現行クライアント実装とサーバー実装の出力を固定値テストで固定し、統合後も同一値を返すことを単体テストで担保する |
| 表示モード導入で既存E2Eのセレクタが壊れる | Medium | 既定モードを現行のツリー表示に固定する（REQ-45.2） |
| 段階1のみ先行リリースで保存が常に409になる | **High** | 段階1と段階3を同一リリースで揃える。分ける場合は暫定ガードを必須とする |
| 深さ無制限での導出計算コスト | Medium | `useMemo` によるキャッシュ＋500項目規模の実測を段階1のリリース前に実施 |
| 実行予算からの参照が切れる | **High** | 差分適用で既存IDを維持する方針を設計の不変条件とし、統合テストで参照維持を検証する |
| 共有 export サービスの変更による他機能への波及 | Medium | `PdfFontService` 等を変更禁止と明記し、差異は本改訂側のラッパで吸収する |

### 設計文書の規模について

追加設計セクション単体で933行、`design.md` 全体で4,441行となった。テンプレートの目安（1,000行）を超えており、機能複雑度が高いことの表れである。spec分割は既にユーザー判断により見送られているため、実装は4段階に分けて `Migration Strategy` に明記した。
