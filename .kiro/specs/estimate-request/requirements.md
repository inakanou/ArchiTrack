...
8. The システム shall ファイルアップロードフィールドの下に構造化データ入力エリアを表示する
9. The システム shall 構造化データ入力エリアに以下のフィールドを持つ明細行を表示する: 名称、規格、単位、数量、単価、金額、備考
10. The システム shall 金額フィールドを入力不可とし、単価と数量の積を自動計算して表示する
11. When ユーザーが数量または単価を変更したとき, the システム shall 該当行の金額を自動再計算する
12. The システム shall 構造化データ入力エリアの下部に全明細行の金額合計を自動計算して表示する
13. The システム shall 明細行の追加ボタンを表示する
14. When ユーザーが追加ボタンをクリックしたとき, the システム shall 新しい空の明細行を末尾に追加する
15. The システム shall 各明細行に削除ボタンを表示する
16. When ユーザーが明細行の削除ボタンをクリックしたとき, the システム shall 該当行を削除し合計金額を再計算する
17. The システム shall Tabキーによるフィールド間の順次移動をサポートする
18. When ユーザーが最終フィールドでTabキーを押したとき, the システム shall 次の明細行の最初のフィールドにフォーカスを移動する
19. When ユーザーが受領見積書名、提出日、およびファイルまたは明細行データを入力して保存したとき, the システム shall 受領見積書を登録する
20. If 必須項目（受領見積書名、提出日）が未入力の場合, then the システム shall バリデーションエラーを表示する
21. The システム shall 1つの見積依頼に対して複数の受領見積書の登録を許可する
22. The システム shall 見積依頼詳細画面に登録済み受領見積書の一覧を表示する
23. The システム shall 受領見積書一覧に受領見積書名、提出日、登録日時を表示する
24. The システム shall 受領見積書の編集機能を提供する
25. The システム shall 受領見積書の削除機能を提供する
26. When ユーザーが受領見積書を削除しようとしたとき, the システム shall 削除確認ダイアログを表示する

### Requirement 13: 受領見積書インラインプレビュー・OCR機能

**Objective:** As a ユーザー, I want アップロードした受領見積書ファイルの内容をインラインプレビューで確認しながらOCRで文字データ化したい, so that 受領見積書の内容入力作業をコピペや一括取り込みで大幅に簡略化できる
...
5. When ユーザーがファイルをアップロードしたとき, the システム shall OCR処理を自動的に開始する
6. While OCR処理が実行中のとき, the システム shall 処理中であることを示すインジケーターを表示する
7. When OCR処理が完了したとき, the システム shall OCR結果をテキストデータとして表示する
8. The システム shall OCR結果のテキストをユーザーが選択・コピーできる状態で表示する
9. The システム shall OCR結果から構造化データ入力エリアへの一括取り込みボタンを表示する
10. When ユーザーが一括取り込みボタンをクリックしたとき, the システム shall OCR結果を解析し、名称・規格・単位・数量・単価を各明細行のフィールドに自動入力する
11. When 一括取り込みが完了したとき, the システム shall 取り込まれた各行の金額を自動計算する
12. If OCR処理が失敗した場合, then the システム shall エラーメッセージを表示し手動入力を促す
13. If OCR結果の解析精度が低い場合, then the システム shall 取り込み結果の確認・修正を促すメッセージを表示する
