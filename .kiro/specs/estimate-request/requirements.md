...
### Requirement 28: 受領見積書明細行 - NET金額入力欄追加

**Objective:** As a ユーザー, I want 受領見積書の登録・編集画面の明細行にNET金額の入力欄を追加したい, so that 値引きや調整後の実際の取引金額を明細行ごとに記録・管理できる

#### Acceptance Criteria

1. The システム shall 受領見積書登録画面の明細行にNET金額入力フィールドを表示する
2. The システム shall 受領見積書編集画面の明細行にNET金額入力フィールドを表示する
3. The システム shall NET金額入力フィールドを金額列の右隣に配置する
4. The システム shall NET金額入力フィールドをユーザーが手動で入力可能なフィールドとする（自動計算ではない）
5. The システム shall NET金額フィールドを任意入力（必須ではない）とする
6. The システム shall NET金額の表示形式を整数表示（小数第1位で四捨五入）とする（Requirement 18の丸め規則に準拠）
7. The システム shall NET金額の入力フィールドにおいてフォーカスアウト時に整数値にフォーマットする
8. The システム shall 構造化データ入力エリアの下部にNET金額合計を自動計算して表示する
9. The システム shall NET金額合計の表示を既存の金額合計の右隣に配置する
10. When OCRまたはデータパースによる一括取り込みが実行されたとき, the システム shall NET金額フィールドを空欄のままとする
11. When 項目選択からの一括転記が実行されたとき, the システム shall NET金額フィールドを空欄のままとする
12. The システム shall NET金額データをデータベースに永続化する
13. The システム shall 受領見積書の編集画面で既存のNET金額データを表示する
