...
4. When ユーザーが顧客名フィールドでフリガナを検索する, the ArchiTrackシステム shall ひらがな・カタカナどちらの入力でも取引先候補を検索する
5. When ユーザーがドロップダウンまたはオートコンプリート候補から顧客を選択する, the ArchiTrackシステム shall 選択された取引先IDをフォームに設定する
6. When ユーザーがドロップダウンまたはオートコンプリート候補から顧客を選択する and 現場住所フィールドが空欄である, the ArchiTrackシステム shall 選択された取引先の住所を現場住所フィールドに自動入力する
7. If ユーザーが顧客を選択した時点で現場住所フィールドに既に値が入力されている場合, then the ArchiTrackシステム shall 現場住所フィールドの値を変更せずそのまま保持する
8. The ArchiTrackシステム shall 営業担当者フィールドのデフォルト値としてログインユーザーの表示名を設定する
9. The ArchiTrackシステム shall 工事担当者フィールドのデフォルト値としてログインユーザーの表示名を設定する
10. When ユーザーが必須項目（プロジェクト名、営業担当者）を入力して「作成」ボタンをクリックする, the ArchiTrackシステム shall 新規プロジェクトをデータベースに保存する
11. When プロジェクトが正常に保存される, the ArchiTrackシステム shall プロジェクト詳細画面に遷移する
12. If ユーザーが必須項目を入力せずに「作成」ボタンをクリックする, then the ArchiTrackシステム shall 入力エラーメッセージを該当フィールドに表示する
13. If プロジェクト名が未入力の場合, then the ArchiTrackシステム shall 「プロジェクト名は必須です」というバリデーションエラーを表示する
14. If プロジェクト名が255文字を超える場合, then the ArchiTrackシステム shall 「プロジェクト名は255文字以内で入力してください」というバリデーションエラーを表示する
15. If 営業担当者が未選択の場合, then the ArchiTrackシステム shall 「営業担当者は必須です」というバリデーションエラーを表示する
16. When プロジェクトが正常に作成される, the ArchiTrackシステム shall 作成日時と作成者を自動的に記録する
17. The ArchiTrackシステム shall プロジェクトに一意のIDを自動付与する
18. If 既に同じプロジェクト名が登録されている場合, then the ArchiTrackシステム shall 「このプロジェクト名は既に使用されています」というバリデーションエラーを表示する
19. The ArchiTrackシステム shall プロジェクト名の一意性チェックを作成時に実行する
...
2. The ArchiTrackシステム shall 各プロジェクトのプロジェクト名、顧客名、営業担当者、工事担当者、ステータス、更新日を一覧に表示する（列順序: プロジェクト名、顧客名、営業担当者、工事担当者、ステータス、更新日）
...
2. When ユーザーが複数のフィルタを適用する, the ArchiTrackシステム shall AND条件で絞り込みを実行する
3. When ユーザーが「フィルタをクリア」をクリックする, the ArchiTrackシステム shall すべてのフィルタを解除し、デフォルト表示（ステータスが「完了」「中止」「失注」のプロジェクトを除外した状態）に戻る
4. The ArchiTrackシステム shall フィルタの選択状態をURLパラメータに反映する
5. When ユーザーがステータスフィルタで「完了」「中止」「失注」のいずれかを選択する, the ArchiTrackシステム shall 選択されたステータスのプロジェクトを一覧に表示する
6. The ArchiTrackシステム shall ステータスフィルタに「完了」「中止」「失注」を含むすべてのステータスを選択肢として提供する
...
5. The ArchiTrackシステム shall プロジェクト名、顧客名、営業担当者、工事担当者、ステータス、更新日のカラムでソート可能とする
...
2. The ArchiTrackシステム shall プロジェクト編集フォームに以下の入力フィールドを表示する: プロジェクト名（必須）、顧客名（任意）、営業担当者（必須）、工事担当者（任意）、現場住所（任意）、概要（任意）
3. When ユーザーが顧客名フィールドで検索テキストを入力する, the ArchiTrackシステム shall 取引先管理機能で登録された取引先を検索フィルタリングして表示する
4. When ユーザーが顧客名フィールドでフリガナを検索する, the ArchiTrackシステム shall ひらがな・カタカナどちらの入力でも取引先候補を検索する
5. When ユーザーがドロップダウンまたはオートコンプリート候補から顧客を選択する, the ArchiTrackシステム shall 選択された取引先IDをフォームに設定する
6. When ユーザーがフォームを編集して「保存」ボタンをクリックする, the ArchiTrackシステム shall 変更内容をデータベースに保存する
7. When プロジェクトが正常に更新される, the ArchiTrackシステム shall 更新日時を自動的に記録し、成功メッセージを表示する
8. If バリデーションエラーが発生した場合, then the ArchiTrackシステム shall エラーメッセージを該当フィールドに表示する
9. When ユーザーが「キャンセル」ボタンをクリックする, the ArchiTrackシステム shall 編集内容を破棄し、詳細表示に戻る
10. If 編集中に他のユーザーがプロジェクトを更新した場合, then the ArchiTrackシステム shall 競合エラーを表示し、最新データの確認を促す
11. If プロジェクト名を変更して既に同じプロジェクト名が登録されている場合, then the ArchiTrackシステム shall 「このプロジェクト名は既に使用されています」というバリデーションエラーを表示する
12. The ArchiTrackシステム shall プロジェクト名の一意性チェックを更新時に実行する（自身のプロジェクトは除外）
...
14. The ArchiTrackシステム shall プロジェクト一覧ページに「ダッシュボード > プロジェクト一覧」のパンくずナビゲーションを表示する
15. The ArchiTrackシステム shall プロジェクト詳細ページに「ダッシュボード > プロジェクト一覧 > [プロジェクト名]」のパンくずナビゲーションを表示する
16. The ArchiTrackシステム shall プロジェクト新規作成ページに「ダッシュボード > プロジェクト一覧 > 新規作成」のパンくずナビゲーションを表示する
17. The ArchiTrackシステム shall プロジェクト編集ページに「ダッシュボード > プロジェクト一覧 > [プロジェクト名] > 編集」のパンくずナビゲーションを表示する
