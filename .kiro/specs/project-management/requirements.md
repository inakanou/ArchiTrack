...
4. When ユーザーが検索を実行する, the システム shall 2文字以上の検索キーワードを要求する
...
4. If バリデーションエラーが発生する, then the システム shall エラーメッセージを該当フィールドに表示する
...
### Requirement 21: ナビゲーション

**Objective:** As a ユーザー, I want プロジェクト一覧画面に素早くアクセスする, so that 業務を効率的に開始できる

**既存実装の活用:** 本要件は既存の`AppHeader`コンポーネントおよび`Dashboard`コンポーネントを拡張して実装する。

#### Acceptance Criteria

1. The システム shall ヘッダーナビゲーション（AppHeader）に「プロジェクト」リンクを表示する
2. When ユーザーがヘッダーの「プロジェクト」リンクをクリックする, the システム shall プロジェクト一覧画面（`/projects`）に遷移する
3. The システム shall 「プロジェクト」リンクを「ダッシュボード」リンクの右側に配置する
4. The システム shall 「プロジェクト」リンクにプロジェクトを表すアイコンを表示する
5. The システム shall ダッシュボードのクイックアクセスセクションに「プロジェクト管理」カードを表示する
6. When ユーザーがダッシュボードの「プロジェクト管理」カードをクリックする, the システム shall プロジェクト一覧画面（`/projects`）に遷移する
7. The システム shall 「プロジェクト管理」カードをクイックアクセスセクションの先頭に配置する
8. The システム shall 「プロジェクト管理」カードに「工事案件の作成・管理」という説明文を表示する
