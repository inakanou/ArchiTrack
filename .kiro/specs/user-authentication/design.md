...
│                                                            │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ ADR-001: データベース選定           2025-01-10  承認済│  │
│  ├──────────────────────────────────────────────────────┤  │
│  │ ADR-002: 認証方式の決定             2025-01-12  承認済│  │
│  ├──────────────────────────────────────────────────────┤  │
│  │ ADR-003: キャッシング戦略           2025-01-14  レビュー中│
│  └──────────────────────────────────────────────────────┘  │
│                                                            │
│  すべてのADRを表示 →                                        │
│                                                            │
...
**要素の配置と仕様**:
- **ヘッダー**:
  - ロゴ（左寄せ）、検索バー（中央）、ユーザー名・設定・ログアウト（右寄せ）
  - 高さ64px、Neutral-50背景、下部にシャドウ
- **ウェルカムカード**:
  - カード形式、軽い影
  - ユーザー名: 24px / Bold
  - 最終ログイン: 14px / Regular、Neutral-600色
  - ロールバッジ: Success-500背景、12px / Medium、白文字
- **クイックアクションカード**:
  - 3列グリッド（デスクトップ）、1列（モバイル）
  - 各カード: 高さ120px、カード形式、軽い影
  - アイコン: 48px、中央寄せ
  - テキスト: 16px / Medium、中央寄せ
  - ホバー時: Primary-50背景、カーソルpointer
- **最近のADRリスト**:
  - リストアイテム: 高さ64px、ボーダー下部
  - ADR番号とタイトル: 16px / Medium
  - 日付: 14px / Regular、Neutral-600色
  - ステータスバッジ: Success-500背景（承認済）、Warning-500背景（レビュー中）
- **すべて表示リンク**:
  - 14px / Medium、Primary色
  - 右寄せ、アイコン付き（→）
...
│                                                            │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ ADR-003: キャッシング戦略           2025-01-14       │  │
│  │                                                      │  │
│  │ Redisを使用したキャッシング戦略の決定...             │  │
│  │                                                      │  │
│  │ ステータス: レビュー中    作成者: 田中太郎            │  │
│  │                                                      │  │
│  │ 📄 詳細  ✏️ 編集  🗑️ 削除                             │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                            │
│  ┌──────────────────────────────────────────────────────┐  │
│  │ ADR-002: 認証方式の決定             2025-01-12       │  │
│  │                                                      │  │
│  │ JWT認証方式の採用決定...                             │  │
│  │                                                      │  │
│  │ ステータス: 承認済        作成者: 山田花子            │  │
...
- **ADRカードリスト**:
  - 各カード: 最小高さ160px、カード形式、軽い影
  - ADR番号とタイトル: 18px / Bold
  - 概要: 14px / Regular、Neutral-600色、2行省略（...）
  - ステータスバッジ: Success-500背景（承認済）、Warning-500背景（レビュー中）、Neutral-500背景（下書き）
  - 作成者: 14px / Regular、Neutral-600色
  - 日付: 14px / Regular、Neutral-500色
  - アクション: 詳細・編集・削除ボタン、32pxアイコンボタン
- **ページネーション**:
