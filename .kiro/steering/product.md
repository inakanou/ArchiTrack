...
_最終更新: 2026-02-12（Steering Sync: 見積書NET案分/利益率ダイアログUI実装、数値表示形式・丸め規則、PDFテキスト抽出ハイブリッド、OCR再実行/リトライ、数量表コピー・タイトル行最適化、オートコンプリート候補キャッシュ、セッション期限切れモーダルを反映）_
...
- ✅ **権限管理ページ**: 管理者向け権限CRUD
- ✅ **プロジェクト管理機能**: プロジェクトCRUD、ステータス遷移管理、担当者割り当て、検索・フィルタリング、ページネーション、取引先選択時の現場住所自動入力
