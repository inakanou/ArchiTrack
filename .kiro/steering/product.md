...
_最終更新: 2026-02-21（Steering Sync: 見積書エクスポート複数行タイプ対応、NET金額モデル改善を反映）_
...
- ✅ **数量表作成機能**: 現場調査結果に基づく数量拾い出し、数量グループ・項目管理、計算方法選択（標準・面積体積・ピッチ）、調整係数・丸め設定、オートコンプリート入力支援（候補キャッシュ最適化、フォーカス時ドロップダウン表示）、フィールドバリデーション（文字数制限・数値範囲・表示書式）、数量表コピー機能、タイトル行表示最適化
- ✅ **内訳書作成機能**: 数量表項目のピボット集計、分類軸（任意分類・工種・名称・規格・単位）によるグループ化、スナップショット独立性、ソート・フィルタリング・ページネーション、楽観的排他制御、Excel出力（.xlsx形式）、クリップボードコピー（タブ区切りテキスト）
- ✅ **見積依頼機能**: プロジェクト紐付き協力業者への見積依頼管理、内訳書項目選択（クライアントサイド状態管理+保存ボタン方式）、分類軸（任意分類・工種）表示、見積依頼文生成（メール/FAX対応）、クリップボードコピー、Excel出力、受領見積書登録（OCR構造化データ抽出・OCR再実行/リトライ機能、PDFテキスト抽出ハイブリッドアプローチ、**Claude Vision API統合によるOCR精度向上**、項目選択からの一括転記、**NET金額は受領見積書単位で管理**、数値表示形式・丸め規則）、ステータス管理（依頼前/依頼済/見積受領済）
- ✅ **自社情報登録機能**: 会社名・住所・連絡先の管理（シングルトンパターン）、見積依頼文への自動挿入、楽観的排他制御
- ✅ **見積書作成機能**: 内訳書からの見積書生成、3行1セット構造（見積金額・実行金額・業者金額）、階層構造管理、受領見積書転記（プロジェクト単位受領見積書取得API）、NET金額案分計算（案分ダイアログUI、合計表示）、利益率適用（利益率ダイアログUI）、諸経費行管理、数値表示形式・丸め規則、**Excel/PDF出力（複数行タイプ選択対応：見積・実行・業者をチェックボックスで選択）**
...
- **包括的なテスト環境**:
  - **単体テスト**: Vitest + supertest/React Testing Library（**Backend 134テストファイル、ブランチカバレッジ80%達成✅**、Frontend 213テストファイル）
  - **要件カバレッジチェック**: 仕様に基づく100%テストカバレッジ検証（E2Eテスト）
  - **統合テスト**: Backend 23テストファイル（API統合、データベース、認証フロー）
...
- **コンポーネントドキュメント**: Storybook 10.xによる視覚的なコンポーネントライブラリとデザインシステム（115ストーリーファイル、インタラクションテスト・アクセシビリティテスト・カバレッジチェック統合）
