...
_最終更新: 2026-06-04（Steering Sync: 数量表の現場調査からの一括グループ生成・水平スクロール時の画像/コメント固定表示、見積依頼文のメーラーへのワンクリック転記を反映）_
...
- ✅ **数量表作成機能**: 現場調査結果に基づく数量拾い出し、数量グループ・項目管理、計算方法選択（標準・面積体積・ピッチ）、調整係数・丸め設定、オートコンプリート入力支援（候補キャッシュ最適化、フォーカス時ドロップダウン表示）、フィールドバリデーション（文字数制限・数値範囲・表示書式）、数量表コピー機能、タイトル行表示最適化、グループ名変更・並び順管理、写真プレビュー・変更ダイアログ（写真重なり解消レイアウト）、現場調査からの数量グループ一括生成（写真枚数分のグループを連番命名で自動作成・写真自動紐づけ）、注釈付き画像レンダリング対応PDF出力、水平スクロール時の画像・コメント固定表示、**数量表インポート機能（Excel/PDFファイルからの数量データ取り込み、Claude Vision OCR連携、フィールドマッピング、プレビュー確認）**
