...
- ✅ **見積依頼機能**: プロジェクト紐付き協力業者への見積依頼管理、内訳書項目選択（クライアントサイド状態管理+保存ボタン方式）、分類軸（任意分類・工種）表示、見積依頼文生成（メール/FAX対応）、クリップボードコピー、Excel出力、受領見積書登録（OCR構造化データ抽出・OCR再実行/リトライ機能、PDFテキスト抽出ハイブリッドアプローチ、項目選択からの一括転記、数値表示形式・丸め規則）、ステータス管理（依頼前/依頼済/見積受領済）
