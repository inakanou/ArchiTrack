...
_最終更新: 2026-03-08（Steering Sync: 数量表拡張・内訳書並び替え・見積書/見積依頼UI改善・テストファイル数更新を反映）_
...
- ✅ **数量表作成機能**: 現場調査結果に基づく数量拾い出し、数量グループ・項目管理、計算方法選択（標準・面積体積・ピッチ）、調整係数・丸め設定、オートコンプリート入力支援（候補キャッシュ最適化、フォーカス時ドロップダウン表示）、フィールドバリデーション（文字数制限・数値範囲・表示書式）、数量表コピー機能、タイトル行表示最適化、グループ名変更・並び順管理、写真プレビュー・変更ダイアログ、注釈付き画像レンダリング対応PDF出力、水平スクロールバー
...
- **包括的なテスト環境**:
  - **単体テスト**: Vitest + supertest/React Testing Library（**Backend 136テストファイル、ブランチカバレッジ80%達成✅**、Frontend 299テストファイル）
...
- **コンポーネントドキュメント**: Storybook 10.xによる視覚的なコンポーネントライブラリとデザインシステム（118ストーリーファイル、インタラクションテスト・アクセシビリティテスト・カバレッジチェック統合）
