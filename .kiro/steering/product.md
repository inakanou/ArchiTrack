...
_最終更新: 2026-02-18（Steering Sync: Claude Vision OCR精度向上、セキュリティ監査対応、テスト規模拡大を反映）_
...
- ✅ **見積依頼機能**: プロジェクト紐付き協力業者への見積依頼管理、内訳書項目選択（クライアントサイド状態管理+保存ボタン方式）、分類軸（任意分類・工種）表示、見積依頼文生成（メール/FAX対応）、クリップボードコピー、Excel出力、受領見積書登録（OCR構造化データ抽出・OCR再実行/リトライ機能、PDFテキスト抽出ハイブリッドアプローチ、**Claude Vision API統合によるOCR精度向上**、項目選択からの一括転記、数値表示形式・丸め規則）、ステータス管理（依頼前/依頼済/見積受領済）
- ✅ **自社情報登録機能**: 会社名・住所・連絡先の管理（シングルトンパターン）、見積依頼文への自動挿入、楽観的排他制御
- ✅ **見積書作成機能**: 内訳書からの見積書生成、3行1セット構造（見積金額・実行金額・業者金額）、階層構造管理、受領見積書転記（プロジェクト単位受領見積書取得API）、NET金額案分計算（案分ダイアログUI）、利益率適用（利益率ダイアログUI）、諸経費行管理、数値表示形式・丸め規則、Excel出力
