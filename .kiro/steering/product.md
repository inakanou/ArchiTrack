...
- ✅ **見積依頼機能**: プロジェクト紐付き協力業者への見積依頼管理、内訳書項目選択、見積依頼文生成（メール/FAX対応）、クリップボードコピー、Excel出力、受領見積書登録（ファイルアップロード）、ステータス管理（依頼前/依頼済/見積受領済）、ステータス変更履歴
- ✅ **会社情報管理機能**: 自社情報のシングルトン管理（会社名、住所、代表者、連絡先等）、適格請求書発行事業者登録番号、楽観的排他制御、見積依頼文への自社情報自動反映

### 開発予定
- 🚧 **見積比較機能**: 受領見積書の比較・分析、業者選定支援
...
- **包括的なテスト環境**:
  - **単体テスト**: Vitest + supertest/React Testing Library（**Backend 1800+テストケース、ブランチカバレッジ80%達成✅**、Frontend 800+テストケース）
