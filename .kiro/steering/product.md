...
_最終更新: 2026-04-08（Steering Sync: テストファイル数・Storybookカウント・依存関係更新を反映）_
...
- ✅ **現場調査機能**: プロジェクトに紐付く現場調査のCRUD、画像アップロード・管理（ストレージ抽象化レイヤー経由、マジックバイトによるMIME判定・拡張子不一致許容）、Canvas注釈編集（Fabric.js: 寸法線・矢印・円・四角形・多角形・折れ線・フリーハンド・テキスト、90度単位画像回転）、PDF報告書エクスポート（jsPDF）、Undo/Redo、自動保存
