...
### テスト・CI環境（Docker Compose、本番相当）

テスト環境（`docker-compose.test.yml`）およびCI環境（`docker-compose.ci.yml`）では、フロントエンドに本番と同じ構成（nginx + ビルド済み静的ファイル）を使用しています。これにより、nginx固有の設定問題（MIMEタイプ、セキュリティヘッダー等）を本番デプロイ前に検出できます。

```
┌─────────────────────────────────────────────────────────────────┐
│  ┌──────────────────┐              ┌─────────────────────┐     │
│  │   Frontend       │              │   Backend API       │     │
│  │   (Nginx/React)  │──────────────│   (tsx watch)       │     │
│  │   Port: 5174/80  │              │   Port: 3100        │     │
│  └──────────────────┘              └──────────┬──────────┘     │
│                                                │                 │
│                                    ┌───────────┴─────────┐      │
│                                    │                     │      │
