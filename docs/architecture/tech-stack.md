...
| | Argon2 | - | パスワードハッシュ |
| | TOTP | - | 二段階認証 |
| **Reporting** | jsPDF | 4.x | クライアントサイドPDF帳票生成 |
| | SheetJS (xlsx) | 0.20.x | Excel（.xlsx）出力 |
| **Testing** | Vitest | 4.x | ユニット・統合テスト |
| | Playwright | 1.62.x | E2Eテスト |
...
### jsPDF 4（クライアントサイドPDF生成）

**選定理由:**
- 帳票生成をブラウザ側で完結でき、サーバーの生成エンドポイントとその負荷を不要にできる
- 未保存の編集内容をそのまま描画できるため、保存前のプレビューが可能
- ベクター描画による罫線・網掛けの精密な制御（A4横の実務書式に追従しやすい）

**用途:**
- フロントエンド生成: 見積書帳票（表紙・内訳書・明細書）、数量表、現場調査報告書、工事写真台帳
- バックエンド生成: 工程表、発注・出来高

**運用上の注意:**
- 日本語フォント資産は初期チャンクから分離し、出力時に遅延ロードする

### SheetJS（xlsx）

**選定理由:**
- ブラウザ・Node.jsの双方で同一APIを利用できる
- 罫線などの装飾に依存しない表構造の出力に十分
- 複数機能でExcel出力ロジックを共通化できる

**用途:**
- フロントエンド生成: 見積書、内訳書、見積依頼
- バックエンド生成: 工程表、発注・出来高
