...
VS Codeの「実行とデバッグ」パネルには、以下のグループに整理された設定が用意されています：

| グループ | 用途 |
|---------|------|
| **1_server** | バックエンドサーバー（ローカル/Docker） |
| **2_unit** | 単体テスト（Vitest） |
| **3_integration** | 統合テスト |
| **4_e2e** | E2Eテスト（Playwright） |
| **5_compound** | 複合構成（Full Stack） |

### 1. Playwright E2Eテストのデバッグ

#### 方法1: VSCode拡張機能を使用（推奨）

1. Playwright拡張機能（`ms-playwright.playwright`）をインストール
2. テストファイルを開く
3. テストエクスプローラーでテストを選択
4. ▶ボタンで実行、または右クリックして「Debug Test」を選択

> テストエクスプローラーを使用すると、個別のテストやテストスイートを簡単に実行・デバッグできます。

#### 方法2: デバッグ設定を使用

1. **F5**キーを押すか、デバッグパネルを開く
2. 以下の設定から選択：
   - **Playwright: E2E (All, Headed)** - 全E2Eテストをブラウザ表示で実行
   - **Playwright: E2E (Current File, Headed)** - 現在のファイルのみ
   - **Playwright: E2E (Debug)** - Playwright Inspector付きでデバッグ
   - **Playwright: E2E (UI Mode)** - UIモードで対話的にテスト
...
1. Vitest拡張機能（`vitest.explorer`）をインストール
2. テストファイルを開く
3. テストエクスプローラーでテストを選択
4. ▶ボタンで実行、またはデバッグアイコンでデバッグ

> テストエクスプローラーを使用すると、watch modeや個別テストの実行が簡単に行えます。

#### 方法2: デバッグ設定を使用

1. デバッグパネルを開く
2. 以下の設定から選択：
   - **Vitest: Backend (Current File)** - 現在のバックエンドテストファイル
   - **Vitest: Frontend (Current File)** - 現在のフロントエンドテストファイル
   - **Vitest: Backend (All)** - 全バックエンド単体テスト
   - **Vitest: Frontend (All)** - 全フロントエンド単体テスト
...
| タスク | 説明 |
|--------|------|
| **Docker: Dev Up** | 開発環境のDockerコンテナを起動 |
