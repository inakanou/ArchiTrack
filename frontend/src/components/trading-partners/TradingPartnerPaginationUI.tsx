/**
 * @fileoverview 取引先一覧 ページネーションUIコンポーネント
 *
 * Task 9.3: ページネーション機能の実装
 *
 * Requirements (trading-partner-management):
 * - REQ-1.5: ページネーションを提供し、1ページあたりの表示件数を選択可能とする
 *   - ページ番号と総ページ数の表示
 *   - 1ページあたり表示件数の選択（10/20/50件）
 *   - ページ切り替え時のデータ再取得
 */
...
/**
 * TradingPartnerPaginationUI コンポーネントのProps
 */
export interface TradingPartnerPaginationUIProps {
  /** ページネーション情報 */
...
/**
 * 表示件数オプション
 * @requirement trading-partner-management/REQ-1.5: 10/20/50件から選択可能
 */
...
function LimitSelect({
  ...
  return (
    <div className="flex items-center gap-2 bg-gray-50 px-3 py-2 rounded-lg">
      <label
        htmlFor="trading-partner-limit-select"
        className="text-sm text-gray-600 whitespace-nowrap"
      >
        表示件数
      </label>
      <select
        id="trading-partner-limit-select"
        value={currentLimit}
...
export default function TradingPartnerPaginationUI({
  ...
  onPageChange,
  onLimitChange,
}: TradingPartnerPaginationUIProps) {
