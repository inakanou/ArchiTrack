...
// Task 101.2 (Req 36.1, 36.2, 36.4): フィット倍率算出の一元化・コンテナ実寸購読・モバイル判定。
// フィット計算は computeFitScale（原寸頭打ち/拡大許容を allowUpscale で制御）へ移行し、
// useElementSize でコンテナ実寸の変化に追従して再フィットする（AnnotationEditor と同一方針）。
...
/**
 * Task 101.2 (Req 33.7, 36.8): 再フィット時に「等倍表示中（＝初期化状態）」とみなすズーム許容誤差。
...
const ImageViewer = forwardRef<ImageViewerRef, ImageViewerProps>(function ImageViewer(
  ...
  // Task 101.2 (Req 36.1, 36.2, 35.7): モバイル幅ではフィット倍率まで拡大許容（allowUpscale=true）、
  ...
  // Task 101.2 (Req 36.4): コンテナ実寸を購読し、アドレスバー伸縮・端末回転・レイアウト変化などの
  ...
  const loadImage = useCallback(
    async (canvas: FabricCanvas, url: string) => {
      ...
      try {
        ...
        // Task 101.2 (Req 36.1, 36.2, 35.7): フィット倍率算出を computeFitScale へ移行。
  ...
  /**
   * Task 101.2 (Req 36.1, 36.2, 36.4, 36.8): コンテナ実寸の変化（モバイルブラウザの
  ...
  useEffect(() => {
    ...
    // 背景画像のスケールを更新し、現在の回転角度を保持したままキャンバス中心へ再配置する
    // （回転挙動は Req 5.3 の現行維持。ImageViewer はキャンバス寸法を回転で入れ替えない）。
    img.scale(scale);
    ...
    controller?.fit();
    // eslint-disable-next-line react-hooks/exhaustive-deps -- 再フィットはコンテナ実寸変化のみを起点とする
