 ...
 *   を数量グループの一番上にのみ表示する
 * - 18.3 / 18.4 / 37.12: 計算用フィールド群（面積・体積／ピッチ）の専用タイトル行（別行）は
 *   描画しない。各計算用フィールドのラベルは EditableQuantityItemRow の操作列右側に
 *   inline 配置された CalculationFields コンポーネント内に行内インラインラベルとして
 *   描画される。本コンポーネントはメイン列（11列 + 操作列 = 計12列）のみを描画する。
 *
