 ...
 * 別途工事の記載欄の番号（51.11）
 *
 * §3 の参照PDFは丸数字 `①`〜`⑤`（U+2460〜U+2464）を用いるが、**本システムの埋め込み
 * フォントはこの5文字を持たない**。`fonts/noto-sans-jp-base64.ts` は Fontsource の
 * `japanese-400-normal` サブセット（6,886 符号位置）で、囲み数字は Google Fonts の
 * 別サブセット（symbols）側にあるため含まれない。丸数字のまま描くとグリフが `.notdef` に
 * 落ち、**紙面には何も出ない**（56.14 で確認した不具合）。
 *
 * 対処として番号の表記を、埋め込みフォントが持つ全角括弧＋全角数字へ改めた。
 * 51.11 が求めるのは「5行分、番号付き」であって丸数字そのものではなく、
 * §11「要件定義で決める論点」8 は「フォントのサブセット化…既存3機能と共有する資産のため、
 * 変更は他機能へも影響する」として、この共有資産へ手を入れることを未決の論点として
 * 留保している。囲みの見た目を保ったまま資産に触れずに済む本表記を採る。
 * 全角で揃えるのは、表紙の数字表記（53.8 / 53.9）が全角である流儀に合わせるため。
 *
 * グリフとして実際に出ることは `__tests__/estimateCoverGlyphRendering.test.ts` が
 * フォント資産の cmap と生成したPDFの双方に対して検査する。
 */
 ...
 * 番号のみとなる（参照PDFの2行目以降と同じ）。5件を超える入力は先頭5件のみを出力する
