...
import type { UploadFailureKind } from './upload.types';
...
export interface BatchUploadError {
   ...
   * 再送可否の区分
   *
   * 分類は送信例外が手元にある時点（`uploadSurveyImages` の catch 内）で行う。
   * 失敗理由の文字列だけでは、サイズ上限超過（413）が文言判定にかからず
   * 再送可へ倒れてしまうため、状態コードを参照できる位置で確定させる
   * （Requirement 37.16, 37.21）。
   */
