 ...
 * - @requirement quantity-table-generation/REQ-48.5: 計算用パラメータ検証を計算方法に応じた規則で行い、
 *   パラメータ形状から計算方法を推測しない（保存キーが方式固有キーのみで復元されることで検証）
 * - @requirement quantity-table-generation/REQ-48.6: 指定計算方法で使用しないフィールドが含まれる場合、
 *   破棄して保存する（旧方式固有キーが保存ボディに残らないことで検証）
 *
 * 設計参照:
