 ...
 *   node scripts/check-coverage-gaps.cjs
 *
 * 終了コード:
 *   0: 問題なし
...
function loadCoverageReport() {
  if (!fs.existsSync(COVERAGE_FILE)) {
    ...
    console.error('   npm run test:unit:coverage を実行してください。');
    process.exit(2);
  }
...
function calculateCoverage(data) {
  ...
  return {
    statements:
      metrics.statements.total > 0 ? (metrics.statements.covered / metrics.statements.total) * 100 : 100,
    branches: metrics.branches.total > 0 ? (metrics.branches.covered / metrics.branches.total) * 100 : 100,
    functions:
      metrics.functions.total > 0 ? (metrics.functions.covered / metrics.functions.total) * 100 : 100,
...
function categorizeFiles(coverageReport) {
  ...
  Object.entries(coverageReport).forEach(([filePath, data]) => {
    ...
    if (
      ...
      relativePath === 'src/redis.ts' ||
      relativePath === 'src/generate-swagger.ts' ||
      relativePath.includes('seed-helpers')
    ) {
      return;
    }
...
function printReport(categories) {
  ...
  const total =
    categories.critical.length + categories.warning.length + categories.belowTarget.length + categories.passing.length;
