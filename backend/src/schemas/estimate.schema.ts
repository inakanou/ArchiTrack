 ...
 *
 * 2000 は Task 57.6 の実測で確定した値（据え置き）。
 * 上限いっぱい（2000項目 ＝ 6000明細行）の最悪経路（全件新規）で一括保存の
 * トランザクション本体を計測した独立3ラン（各 n=20）の観測最悪値は
 * 4244ms / 3919ms / 3593ms、Task 57.2 の2ラン（3513ms / 2733ms）と合わせて最悪 4244ms。
 * 発行クエリ数は件数に依らず有界（新規経路10文＋分割で最大+3）で、件数に比例して
 * 増えるのは1文あたりのパラメータ数だけであるため、上限を下げる必要はないと判断した。
 * 代わりにトランザクションの制限時間を実測に基づいて明示する
 * （`estimate-draft.service.ts` の `ESTIMATE_SAVE_TRANSACTION_OPTIONS`）。
 * この上限を引き上げる場合は、同じ最悪経路で再計測して制限時間も見直すこと。
 */
